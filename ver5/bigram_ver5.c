...
int simpleHashFunction(const char* str, int s) {
    int len = strlen(str);
    unsigned long hash = 0;

    if (len > 0) {
        hash += str[0];             // 첫 번째 문자
        hash += str[len / 2];       // 중간 문자
        hash += str[len - 1];       // 마지막 문자
    }
...
void addBigramToHashTable_recurcive(HashTable* table, const char* bigram) {
    int index = simpleHashFunction(bigram, table->size);
...
void addBigramToHashTable(HashTable* table, const char* bigram) {
    int index = simpleHashFunction(bigram, table->size);
    ...
    // 이전에 같은 bigram이 존재하는지 확인하고 빈도수 업데이트
    Node* current = newNode->next;
    while (current != NULL) {
        if (strcmp(current->bigram, bigram) == 0) {
            newNode->frequency = current->frequency + 1;
...
void processTextToWordBigrams(HashTable* table, const char* text) {
    ...
    while (word != NULL) {
        ...
        addBigramToHashTable(table, bigram);
...
// 메인 함수 예제
int main() {
    const char* filename = "TheCompleteWorksOfWilliamShakespeareFullver.txt";
